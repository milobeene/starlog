...
> ## ⚠️ v1.0에서 엔드포인트가 줄고 는다 — 2026-08-27
>
> **사라짐**: `/api/auth/**` 전부 · `/api/admin/system` · `/api/me/quota` ·
> 탈퇴·복구 · 구글 연동 · 이메일 인증 · 비밀번호 재설정
> **모양 바뀜**: `/api/admin/games/**` → 일반 기능으로 승격
> **새로 생김**: 커버 직접 업로드(멀티파트) · 커버 스트리밍 · 커버 위치 이전 ·
> 마스터 일괄 동기화 · 미디어 · 진단
>
