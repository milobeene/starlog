...
| 결정 | 근거 |
|---|---|
| **backlog_entry_tag 삭제 → backlog_entry.tag_id** (2026-08-26 개정) | 태그가 항목당 하나로 확정됐다(스펙 §6.7 v1.6). 조인 테이블·`uk_backlog_entry_tag`·FK 2개가 사라지고 nullable FK 1개 + `(member_id, tag_id)` 인덱스가 들어왔다. 테이블 25 → 24, FK 34 → 33 |
...
- 전체 테스트 409개 ✅ (V2DataMigrationTest 삭제로 -1) → 태그 단일화 후 **420개** ✅
