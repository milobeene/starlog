...
### 6-1. IGDB 키는 **사용자가 자기 것을 넣는다** [2026-08-26 결정]
...
- **DB는 H2 파일 모드.** dev/test가 이미 H2를 PostgreSQL 호환 모드로 돌려서 Flyway·JPQL이
  그대로 산다. SQLite로 가면 dialect부터 다시 검증해야 해서 얻는 것보다 비용이 크다.
