...
> ## ✅ 이 문서는 닫혔다 — 2026-08-28
>
> 🔴 항목이 **8단계에서 전부 소진됐다.** 쿼터(`common/quota/` 11파일) · 감사 로그 ·
> `/admin` 시스템 탭 · 회원 목록 · 가입 승인 · 탈퇴 유예가 모두 사라졌고,
> `@Profile("!desktop")` 게이트도 함께 없어졌다 (막을 웹 빌드가 없다).
> 남은 이력은 `v1.0-roadmap.md` 부록 B.

> ## ✅ 🔴 항목의 대부분을 4단계에서 실제로 걷어냈다 — 2026-08-27
>
> 이메일/구글 로그인 · 세션 · CSRF · CORS · 이메일 인증 · 비밀번호 재설정 · 메일 발송 ·
> 개발용 헤더 인증(→ 테스트 장치로 성격 변경) · 가입 승인 · 이메일 허용목록
>
> **남은 🔴**: 감사 로그 · 일일 쿼터 · `/admin` 화면 · 탈퇴 유예(백엔드) · 정리 배치 크론
> → 8단계 "정리와 승격"에서.
>
> 자세한 내역은 `v1.0-plan.md` §4.
