...
> **떼기 쉬운 모양으로** 짓는다.
>
> ## ⚠️ 전제가 세 번 바뀌었다. 최종은 아래다
>
> | | |
> |---|---|
> | **죽는 것** | Render · Vercel |
> | **사는 것** | **Neon · R2 · IGDB — 라이브로 그대로** |
> | 사용자 | 나 혼자, **윈도우 PC + 맥북 두 대**. 폰 안 씀 |
> | 배포 | 안 뿌린다. 달라는 사람 있으면 저장소를 준다 |
>
> **처음 이 문서를 쓸 때 깔았던 "1인 1기기, 데이터도 로컬 파일" 전제는 폐기됐다.**
> 기기가 둘이라 Neon을 남기는 쪽이 맞다 — 로컬 파일이면 두 기계 사이에서 백업을
> 주고받아야 하는데, DB 파일을 클라우드 동기화 폴더에 두면 **충돌이 아니라 손상**이 난다.
>
> 그 결과 아래 §6의 "IGDB 키를 사용자가 넣는다", "동기화가 문제다" 항목이 **무효**가 됐다.
> 서버 설정이 그대로 살아서 IGDB도 R2도 손댈 게 없다.
>
> 자세한 계획은 `docs/v1.0-plan.md`, 미결은 `docs/v1.0-decisions.md`.
