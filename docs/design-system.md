...
**대상은 아이폰 14(390×844)부터 1920까지.** 폰을 실제로 쓰진 않지만, 폭 하한을 낮춰야
일렉트론 창을 화면 반쪽에 놓고 쓸 수 있다.

### 여백은 `globals.css` 한 곳에서만 정한다

| 클래스 | 하는 일 |
|---|---|
| `.page-x` | 좌우 여백 — `1rem → 1.5rem → 2rem → 2.5rem` (base/sm/lg/xl) |
| `.page-top` | 헤더 아래 여백 — `4.5rem → 5.5rem → 6rem → 7rem` |
| `.page-inset-l/r` | absolute 요소를 `.page-x`와 같은 선에 맞출 때 |

**페이지마다 `px-8` / `pt-24`를 직접 쓰지 않는다.** 예전엔 그랬는데 390px에서
좌우 5rem을 먹어 본문이 반으로 줄었다. lg 이상 값은 예전과 같아 데스크탑은 그대로다.

### 브레이크포인트별로 바뀌는 것

| | 폰 (base) | sm 640 | md 768 | lg 1024 | xl 1280 |
|---|---|---|---|---|---|
| 헤더 | 압축 메뉴, 심볼 없음 | 닉네임 표시 | 심볼 + 정식 nav | 슬롯 `w-44` | |
| 라이브러리 사이드바 | **드로어** (좌하단 [목록] 버튼) | | | **고정 표시** | `w-72` |
| 게임 그리드 | 2열 | 3열 | 4열 | 6열 | 8열 |
| 대시보드 타일 | 1열 | 2열 | | 4열 | |
| 상세 통계 | 2열 | | 4열 | | |
| 필터 | 1열 | 2열 | | 3열 | 5열 |
...
- **헤더를 absolute로 짜면 폰에서 겹친다.** absolute끼리는 서로를 밀어내지 못한다.
  컨테이너를 flex로 바꾸고, 심볼의 정확한 중앙 정렬만 md 이상에서 absolute로 남겼다
- **`<canvas>`는 대체 요소라 `inset-0`으로 늘어나지 않는다.** CSS 폭이 auto면
  `width` 속성값을 그대로 CSS 픽셀로 쓴다 — `h-screen w-screen`을 반드시 같이 준다
  (파티클이 오른쪽 화면 밖에서 터지던 버그가 이것이었다)
