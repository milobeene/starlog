...
### 4-0. 입구 로딩 연출 + 폰트  [스펙 확정, 스타일 미정 — 반응형과 같이]

**로딩 연출** (`(public)/page.tsx`). 지금은 세션 판정 중 버튼 자리가 비어 있다가 툭 나타난다.
Render 무료의 콜드스타트(최대 50초+)에서 특히 길다.

- **처음부터 바로 뜬다.** 지연 노출(0.5초 뒤 등장 같은 것)은 하지 않는다
- **0.5초 안에 응답이 와도 1~2초는 유지**한다 — 깜빡이면 더 거슬린다
- 끝날 때 **자연스럽게** 전환 (툭 사라지지 않게)
- **다른 페이지에 있다가 입구로 돌아오면 애니메이션 없음** — 세션 정보가 이미 있어서 판정할 게 없다
- 스타일·문구는 미정. 배경 FluidBackground와 어울리는 선에서 그때 정한다

**폰트**: 숫자는 모노, 나머지는 미정. 반응형 작업과 함께.
