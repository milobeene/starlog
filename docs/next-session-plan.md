...
## 우선순위 요약 (2026-08-26 저녁 갱신)

```
✅ 완료   태그 단일화 · 폴더명 · Neon V1/V2 · O-4 세션 DB화 · O-5 풀 튜닝 · 배포
─────────────────────────────────────────────────────────────
1. 커버 업로드 검증 (R2 CORS)     ★ 실데이터 넣기 전에 끝내야 한다
2. 실데이터 투입                   [결정 필요 — 아래]
3. 반응형 4곳 + 사이드바 태그 그룹 + 입구 로딩 + 폰트 → PWA
...
- **1번이 먼저인 이유** — 배포 환경에서만 드러나는 부류다. 실데이터를 먼저 넣으면
  커버 없는 항목이 쌓인 뒤에 발견하게 되고, 되짚는 비용이 붙는다.
- **2번이 3번보다 먼저인 이유** — 반응형·성능은 **데이터가 있어야 진짜로 검증된다.**
  게임 3건으로는 스크롤도, 그리드 램프도, 사이드바 그룹도 눈으로 확인이 안 된다.
- **4번을 뒤로 미룬 이유** — IGDB 쿼터는 **사용자가 여럿일 때** 의미가 생긴다.
  지금은 혼자라 초당 4건 한도에 닿을 일이 없다. 지인을 초대하기 직전이 적기다.
