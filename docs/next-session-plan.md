...
> 현재 상태: **배포 완료.** 백엔드·프론트·DB 모두 살아 있고 구글 로그인까지 통했다.
> 테스트 421개 초록. Neon은 V2까지 적용(테이블 26개) + 관리자 1명·기본 선택지 시딩됨.
> → 남은 것은 **커버 업로드 검증 → 실데이터 → 반응형** 순이다.
>
...
## 작업 2. ~~배포~~ — ✅ **2026-08-26 완료** (2-6 커버 업로드만 미검증)
...
### 2-6. 배포 후 종단 검증  ★ **여기부터 남았다**

| 항목 | 상태 |
|---|---|
| 구글 로그인 → 연동 → 프로필 | ✅ 확인 (DB도 대조함) |
| 이메일 로그인 | ✅ 확인 |
| 담기 → 회차 → 통계 | ⬜ 미확인 |
| **커버 업로드** | ⬜ **미확인 — 제일 위험** |

**커버 업로드가 위험한 이유** — 브라우저가 R2로 **직접 PUT**하는 구조라, R2 버킷의 CORS가
Vercel 도메인(`https://starlog-xi.vercel.app`)을 허용해야 한다. 이건 Render 환경변수가 아니라
**Cloudflare R2 대시보드**에서 버킷 CORS 정책을 고치는 일이다 (사용자 작업).

배포 환경에서만 드러나는 부류라 **실데이터를 넣기 전에** 확인해야 한다 — 나중에 발견하면
커버 없는 항목이 쌓인 뒤라 되짚는 비용이 든다.
...
| | 지금 값 | 뜻 |
|---|---|---|
| `colorA`~`colorE` | `#1E6349` `#D98E5E` `#C4DCC0` `#35876A` `#0B1A14` | 기조 / 밝은 대비 / 옅은 색 / 중간 강조 / 어두운 바닥 |
| 흐름 속도 (입구 / 앱) | `0.1` / `0.067` | 유체가 흐르는 빠르기 |
| 순회 주기 | `0.08` | 색이 한 바퀴 도는 시간 |
| 무지개 채도 | `0.45` | 색이 얼마나 진한가 |
| 밝기 폭 | `0.34~0.66` | 지각 밝기 평탄화 범위 |
...
- **프리셋 + 커스텀 둘 다.** 프리셋만으로는 답답하고, 커스텀만 있으면 망치기 쉽다.
  ...
  (원본 오렌지 / 에메랄드 / 이끼와 황동 / 심해 / 숲과 노을 / 한 톤 초록)
...
✅ 완료
   ...
   실데이터 76건 임포트 · 입구 로딩 연출 · 배경 팔레트 · UI 3건 · 사이드바 태그 그룹

── v0.1 남은 것 ────────────────────────────
1. 커버 업로드 종단 확인   R2 CORS는 설정·검증 끝. 화면에서 실제 업로드만 미확인
2. 폰트 (숫자 모노)

── v0.1 이후 ──────────────────────────────
3. 배경 색상 설정 (작업 4-2)
4. IGDB 안내 + 일일 쿼터 + /admin 시스템 탭   ← 지인 초대 직전
5. 반응형 + PWA                              ← **웹 유지가 정해진 뒤에만**

── v1.0 ──────────────────────────────────
6. 로컬 일렉트론 앱 + 세이브파일 동기화
```
...
**폐기**: 옵시디언 임포터, 알려진 한계 3건 수정, 콜드스타트 우회 핑.

---

## 미결 하나 — 실데이터를 어떻게 넣나  [결정 필요]

임포터는 만들지 않기로 했고(작업 3), Neon은 지금 **게임 0건**이다. 그런데 "화면에서 직접 담는다"의
실제 무게가 계획에 안 잡혀 있다 — 76건을 IGDB 검색 → 담기 → 회차 → 취득까지 손으로 넣으면
항목당 1~2분만 잡아도 **두 시간이 넘는다.**

### 선택지

| 안 | 내용 | 비용 | 잃는 것 |
|---|---|---|---|
| **A. 손으로 조금씩** | 생각날 때마다 몇 건씩. 지금 확정된 안 | 0 (분산됨) | 반응형·성능 검증이 계속 미뤄진다 |
| **B. 관리자 전용 일괄 담기 API** | 게임명 목록을 붙여넣으면 IGDB 검색 1위로 자동 담기 | 반나절 | IGDB 1위가 틀린 건 나중에 손으로 고쳐야 |
| **C. 검증용 시드만** | 반응형 확인용으로 30~40건만 빠르게, 나머지는 A | 1시간 | — |

**아직 정하지 않았다.** 3번(반응형)에 들어가기 전에 정하면 된다.
