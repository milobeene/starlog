...
**v0.1(웹) 완료 · `v0.1` 태그.** 지금은 `v1.0` 브랜치에서 **데스크탑 앱**을 만든다.
v1.0은 "내 앱을 데스크탑으로"가 아니라 **"각자 자기 인프라의 주인이 되는 프로그램"**이다 —
로그인 없음, DB·스토리지·IGDB 키를 사용자가 고른다.

⚠️ **`main`은 이제 v1.0과 같은 자리다** (2026-08-28, fast-forward). GitHub이
`workflow_dispatch` 버튼을 **기본 브랜치에 있는 워크플로에만** 띄우기 때문이다.
옛 웹 서비스 상태는 **`v0.1` 태그**(`7396d89`)에 그대로 있다 — 태그는 안 움직인다.
작업은 계속 `v1.0`에서 하고, 배포 시점에 `main`을 fast-forward 한다.

## 참조 문서 (필요할 때 읽을 것, 미리 다 읽지 말 것)

- `docs/v1.0-roadmap.md` — **전체 단계(1~10)와 진행 상황. "다음 뭐 하지"는 여기.**
...
**[최근 접속]이 즉시 복귀**하도록 죽이지 않는다. 대신 **새로 띄우기 전에 반드시 죽이고
`await`한다** — 살아 있는 놈이 `.mv.db`를 쥔 채 같은 파일을 열면 `DB_IN_USE`고,
신호만 보내고 넘어가면 잠금이 풀리기 전에 열어 **간헐적으로** 실패한다.

### 백업은 일렉트론이 파일 복사로 한다 (9단계)

`BACKUP TO`(zip)를 안 쓴다 — **DB가 닫혀 있는 순간(입구 화면)이 있어서** 그냥 복사하면 되고,
그래야 백업이 세이브파일과 **같은 `.mv.db`**로 남아 되돌릴 때 압축 해제가 없다.
`desktop/backup.js`. 열 때 자동(내용이 바뀌었을 때만) + 입구에서 수동. **30개 / 100MB.**
...
## 미디어 — 커버와 스크린샷 (v1.0 6·7단계)

**커버는 위치를 한 장 한 장이 들고 있다** (`CoverImage.location = LOCAL | EXTERNAL`).
앱 전체 설정으로 두면 설정을 바꾸는 순간 이미 있는 커버가 전부 깨진다.

업로드 경로가 **둘로 갈린다.** 어느 쪽인지는 **서버가 정한다** —
`POST /api/backlog/{id}/cover/upload-url`이 `mode`를 답해주고 화면이 따라간다
(`frontend/src/lib/upload.ts`).

| | EXTERNAL | LOCAL |
|---|---|---|
| 흐름 | 허가증 → 브라우저가 S3에 PUT → 확정 (3왕복) | 백엔드로 파일 전송 (1왕복) |
| 조회 | 버킷 공개 URL | `/api/backlog/{id}/cover/file?v={파일명}` |

**무엇을 스토리지에 올릴지는 연결 설정의 체크박스 둘**이다 (`starlog.media.*`).
자격증명이 없으면 체크가 켜져 있어도 무시한다 (`MediaTargets`).

**스크린샷은 DB에 행이 없다.** `<데이터루트>/media/<slug>/`의 폴더가 곧 목록이다 —
캡션도 순서도 안 주기로 했고, 탐색기 열기를 주는 이상 사람이 직접 지운다.
⚠️ 폴더 이름이 겹치면 게임 번호를 붙인다 — **같은 이름의 게임이 둘 있을 수 있다.**

## 시스템 화면과 API 사용량 (v1.0 8단계)

`/admin`이 **`/system`**이 됐다. 탭 셋 — 사용량 · 게임 마스터 · 연결.

**외부 API 호출은 행으로 남는다** (`api_call_log`, 30일 보관). 한도가 "초당 4회"처럼
전부 기간당 횟수라 누적 카운터로는 비교가 안 된다. **성공·실패를 안 가린다.**
한도 숫자는 서버가 아니라 `frontend/src/lib/apiLimits.ts`가 **기준일과 함께** 들고 있다.
