...
import com.milobeene.gamebacklog.support.FakeFileStorage;
...
class CoverImageTest extends ControllerTestSupport {
    ...
    public void 상세에서도_커버가_resolved_안에서_폴백된다() throws Exception {
        ...
        game.syncFromCatalog(new com.milobeene.gamebacklog.game.domain.CatalogSyncCommand(
    ...
    public void 스토리지_장애_메시지가_게임_DB_장애와_구분된다() throws Exception {
        ...
        Member member = saveMember();
        Long entryId = addEntry(member, saveGame("Celeste"));
        storage.willFail(new com.milobeene.gamebacklog.common.exception.ExternalApiException(
                com.milobeene.gamebacklog.common.exception.ExternalApiException.Service.FILE_STORAGE,
                "버킷 연결 실패"));
