...
/**
 * 백로그에 담기 (FR-BL-01). 마스터 게임 id 또는 RAWG id 중 하나를 받는다 (J-3).
 *
 * 검색 결과가 두 종류라서 이렇게 됐다 — 이미 마스터에 있으면 gameId가 실려 오고,
 * RAWG에만 있으면 rawgId가 온다. 프론트는 검색 응답의 두 필드를 그대로 넘기면 되고,
 * "이미 캐시됐는지"를 판단하지 않는다. 그 판단은 GameResolver가 한다
 */
public record BacklogAddRequest(Long gameId, String rawgId) {
     ...
     */
    @AssertTrue(message = "gameId 또는 rawgId 중 하나는 필요합니다")
    public boolean isTargetPresent() {
        return gameId != null || TextValues.normalize(rawgId) != null;
    }
}
