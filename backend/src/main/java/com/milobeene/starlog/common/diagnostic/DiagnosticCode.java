...
public enum DiagnosticCode {
    ...
    /**
     * 세이브파일이 손상됐다 (2026-08-28).
     *
     * **실제로 났고, 그때 화면에는 "앱 서버가 예기치 않게 종료됐습니다"만 떴다.**
     * 성한 백업이 12개나 있었는데 되돌리라는 말이 어디에도 없었다 — 사용자는 데이터가
     * 통째로 날아간 줄 알았다. `DB_UNKNOWN_ERROR`에 섞여 있으면 안 되는 이유가 이것이다.
     *
     * H2는 잘린 파일을 열 때 `IO Exception ... [90028]`을 던진다(내부는 EOFException).
     * 이 코드를 받으면 일렉트론이 **백업 목록으로 바로 보낸다** — 할 일이 정해져 있는
     * 유일한 실패라서, 안내만 띄우고 끝내면 안 된다
     */
    DB_CORRUPTED;
