...
public class Member extends BaseEntity {
    ...
    /**
     * 연결 해제 (FR-AUTH-08, BR-AUTH-01) — **비밀번호가 있을 때만 허용한다.**
     *
     * 한때 통째로 막아뒀다. 이메일 가입과 비밀번호 설정을 둘 다 닫아둔 상태라(인증 메일을
     * 보낼 수 없다) 해제를 허용하면 로그인 수단이 하나도 안 남는 계정이 생길까 봐서였다.
     *
     * 그 걱정은 **비밀번호가 없는 계정에만** 해당한다. 비밀번호가 있으면 해제해도
     * 이메일 로그인이 남으므로 잠기지 않는다 — 원래 규칙(BR-AUTH-01)이 정확했다.
     *
     * "이메일 인증됨"을 기준으로 삼지 않는 이유 — 구글로 가입하면 이메일은 인증된 상태인데
     * 비밀번호는 없다. 그 계정을 풀면 로그인 수단이 0개가 된다. 인증 여부가 아니라
     * **로그인 수단이 남는가**가 기준이다
     */
    public void unlinkGoogle() {
        if (!hasPassword()) {
            throw new InvalidInputException(
                    "비밀번호가 없어 연결을 해제하면 로그인하실 수 없습니다. "
                            + "계정을 정리하시려면 회원 탈퇴를 이용해 주세요");
        }
