...
/*
 * 팔레트 (2026-08-26 개편). 기조를 오렌지 → 에메랄드로 옮기고 나머지를 맞췄다.
 *
 * **원본의 역할 구조는 그대로 뒀다** — 기조 → 밝은 따뜻한 빛 → 옅은 차가운 색 →
 * 중간 채도 강조 → 어두운 바닥. 이 다섯이 거리(d1·d2·d3)에 따라 겹치면서 깊이를 만든다.
 * 전부 초록으로 칠하면 구조가 사라지고 단색 그라디언트가 된다 — B의 따뜻함을 남긴 이유다.
 *
 * 원본 값 (되돌릴 때 참고):
 *   A #E85B05  23° 96% 46%  오렌지
 *   B #E8C46B  43° 73% 66%  모래빛
 *   C #A8D8DB 183° 42% 76%  연하늘
 *   D #289E8E 172° 59% 39%  청록
 *   E #1C3556 214° 51% 22%  남색
 */
vec3 colorA = vec3(0.129, 0.420, 0.325);  // #216B53 160° 53% 27%  에메랄드 — 기조색
vec3 colorB = vec3(0.834, 0.772, 0.526);  // #D4C486  48° 48% 68%  바랜 금빛 — 유일한 따뜻함
vec3 colorC = vec3(0.688, 0.872, 0.805);  // #AFDECD 158° 42% 78%  옅은 민트
vec3 colorD = vec3(0.180, 0.620, 0.532);  // #2D9E87 168° 55% 40%  중간 청록 — 강조
vec3 colorE = vec3(0.060, 0.160, 0.136);  // #0F2822 166° 45% 11%  짙은 초록 — 바닥
